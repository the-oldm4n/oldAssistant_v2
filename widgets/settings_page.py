...
from mygui import main_apply_colors, CustomSvgWidget, CustomToggle, ColorSettingsWindow, color_signal, update_presets_signal
...
from path_builder import get_path, get_app_data_dir, get_full_filepath
...
from PySide6.QtWidgets import QPushButton, QLabel, QVBoxLayout, QHBoxLayout, QApplication, QWidget,\
    ...
    QListWidgetItem, QCheckBox, QSizePolicy, QGridLayout, QFileDialog
...
class SettingsPage(QWidget):
    ...
    def init_ui(self):
        ...
        speech_hook_widget = SpeechHookManagerWidget(self.main, self)
        interface_widget = InterfaceWidget(self.main, self)
        ...
        self.content_container.add_page(interface_widget)
        ...
        buttons_data = [
            ...
            },
            {
                "key": "interface",
                "text": "Интерфейс",
                "icon_path": self.main.icon_styles_path,
                "tooltip": "Настройки интерфейса",
                "index": 3,
                "widget": interface_widget
            },
            {
                "key": "panel",
                "text": "Панель",
...
class InterfaceWidget(QWidget):
    ...
    def load_custom_styles(self):
        ...
        if os.path.exists(custom_presets):
            try:
                ...
                debuglog.error(f"[SETTINGS-WIDGET] Ошибка чтения кастомных стилей: {e}")
    ...
    def load_styles_from_folder(self, folder_path, container_layout, is_custom=False):
        ...
        try:
            ...
            debuglog.error(f"[SETTINGS-WIDGET] Ошибка чтения папки {folder_path}: {e}")
            return
    ...
    def apply_style_from_button(self, file_path, filename, is_custom=False):
        ...
        try:
            ...
            debuglog.error(f"[SETTINGS-WIDGET] Ошибка применения стиля {filename}: {e}")
            self.main.show_notification(f"Ошибка: {str(e)[:50]}...")
    ...
    def create_style_button(self, filename, folder_path, is_custom=False):
        ...
        try:
            ...
            debuglog.error(f"[SETTINGS-WIDGET] Ошибка создания кнопки для {filename}: {e}")
    ...
    def apply_style_file(self, filename):
        ...
        if os.path.exists(custom_path):
            ...
            debuglog.error(f"[SETTINGS-WIDGET] Пресет '{filename}' не найден ни в одной из папок.")
            return
        ...
        try:
            with open(preset_path, 'r', encoding='utf-8') as json_file:
                ...
                with open(self.main.color_path, 'w') as f:
                    json.dump(styles, f, indent=4)

                self.main.styles = styles
                self.main.apply_styles()
                color_signal.color_changed.emit()
                self.main.show_notification("Стиль успешно применен!")
                debuglog.info(f"[SETTINGS-WIDGET] Применён стиль из файла: {filename}")

        except json.JSONDecodeError:
            logger.error(f"Ошибка: файл пресета повреждён ({preset_path}).")
            debuglog.error(f"[SETTINGS-WIDGET] Ошибка: файл пресета повреждён ({preset_path}).")
        except Exception as e:
            logger.error(f"Ошибка загрузки пресета: {e}")
            debuglog.error(f"[SETTINGS-WIDGET] Ошибка загрузки пресета: {e}")
    ...
    def edit_style_name(self, filename, folder_path, button):
        ...
        try:
            if os.path.exists(new_file_path):
                self.main.show_notification(f"Файл с именем '{new_name}' уже существует!")
                return
            ...
            self.main.show_notification(f"Название изменено на: {new_name}")
            
        except FileNotFoundError:
            debuglog.error(f"[SETTINGS-WIDGET] Файл не найден: {old_file_path}")
            self.main.show_notification("Ошибка: файл не найден!")
        except PermissionError:
            debuglog.error(f"[SETTINGS-WIDGET] Нет прав на переименование файла: {old_file_path}")
            self.main.show_notification("Ошибка: нет прав на изменение файла!")
        except Exception as e:
            debuglog.error(f"[SETTINGS-WIDGET] Ошибка при переименовании файла: {e}")
            self.main.show_notification(f"Ошибка: {str(e)}")
    ...
    def delete_custom_style(self, filename, folder_path):
        ...
        if reply != QMessageBox.StandardButton.Yes:
            debuglog.info("[SETTINGS-WIDGET] Удаление стиля отменено")
            return
        
        if reply == QMessageBox.StandardButton.Yes:
            try:
                os.remove(file_path)
                self.main.show_notification(f"Стиль '{filename.replace('.json', '')}' удален")
                self.load_custom_styles()
                update_presets_signal.presets_updated.emit()
                
            except Exception as e:
                debuglog.error(f"[SETTINGS-WIDGET] Ошибка удаления стиля {filename}: {e}")
                self.main.show_notification(f"Ошибка удаления: {str(e)[:50]}...")

    def open_color_settings(self):
        """Открывает диалоговое окно для настройки цветов."""
        try:
            color_dialog = ColorSettingsWindow(self.main)
            color_dialog.colorChanged.connect(self.main.apply_styles)
            color_dialog.exec()
        except Exception as e:
            logger.error(f"Ошибка при открытии окна настроек цветов: {e}")
            debuglog.error(f"[SETTINGS-WIDGET] Ошибка при открытии окна настроек цветов: {e}")
            self.main.show_message(f"Не удалось открыть настройки цветов: {e}", "Ошибка", "error")
...
class SettingsWidget(QWidget):
    ...
    def set_default_settings(self):
        ...
        with open(self.main.settings_file_path, "w", encoding="utf-8") as file:
            ...
            self.main.show_notification("Установлены настройки по умолчанию!")
    ...
    def apply_settings(self):
        ...
        self.main.save_settings()
        self.hide_method()
        self.main.show_notification(message="Настройки применены!")
...
class OtherSettingsWidget(QWidget):
    ...
    def add_link_desktop(self):
        ...
        if not os.path.exists(executable_path):
            self.main.show_notification(f"Ошибка: Файл {executable_path} не существует")
            return
        ...
        try:
            ...
            self.main.show_notification(f"Ошибка создания ярлыка на рабочем столе: {e}")
        ...
        try:
            ...
            self.main.show_notification(f"Ошибка создания ярлыка в меню 'Пуск': {e}")

        # Показываем сообщение только если были созданы новые ярлыки
        if shortcuts_created:
            self.main.show_notification("Ярлык успешно создан!")
        else:
            self.main.show_notification("Ярлык уже существует!")
    ...
    def get_devices(self):
        ...
        try:
            ...
            self.device_list.addItem("Нет активных микрофонов")
            self.main.show_notification(f"Ошибка при получении данных аудиоустройств: {str(e)}")
...
class SpeechHookManagerWidget(QWidget):
    ...
    def add_new_word(self):
        ...
        if new_word in self.user_keywords[self.current_list]:
            self.main.show_notification("Такое слово уже есть в списке!")
            return
    ...
    def reset_to_default(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            self.main.show_notification("Команды сброшены к значениям по умолчанию!")
