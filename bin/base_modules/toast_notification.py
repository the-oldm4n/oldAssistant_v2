...
from PySide6.QtWidgets import (QLabel, QGraphicsColorizeEffect, QHBoxLayout, QWidget, QVBoxLayout,
                               QDialog, QMessageBox, QPushButton, QCheckBox)
...
class SimpleNotif():
    def __init__(self, parent=None, message="", title="Уведомление", message_type="info",
                 buttons=QMessageBox.StandardButton.Ok, custom_buttons=None,
                 show_checkbox=False, checkbox_text="Запомнить мой выбор", checkbox_default=False):
        self.parent = parent
        ...
        self.style_manager = main_apply_colors
        self.styles = self.style_manager.load_styles()
        self.checkbox_state = checkbox_default
        self.result = None
        self.main = None
        self.drag_pos = None
        self.container = None
    ...
    def init_ui(self):
        ...
        if self.show_checkbox:
            ...
            self.checkbox.setObjectName("RememberCheckbox")
            self.checkbox.setChecked(self.checkbox_default)
    ...
    def create_buttons(self):
        ...
        if self.custom_buttons:
            for text, value in self.custom_buttons:
                btn = QPushButton(text)
                btn.setObjectName("DialogButton")
                btn.setFixedSize(100, 35)  # Чуть больше для длинного текста
                # Передаем значение через лямбду
                btn.clicked.connect(lambda checked, val=value: self.custom_button_clicked(val))
    ...
    def custom_button_clicked(self, value):
    ...
    def button_clicked(self, button_role):
        """Обработка стандартной кнопки"""
        self.result = button_role
        if self.show_checkbox:
            self.checkbox_state = self.checkbox.isChecked()
    ...
    def exec_(self):
        ...
        if result == QDialog.DialogCode.Rejected:
            return None
        return self.result if self.result is not None else result
