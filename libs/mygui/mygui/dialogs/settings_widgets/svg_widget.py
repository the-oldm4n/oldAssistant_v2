...
class SvgWidget(QWidget):
    ...
    def init_ui(self):
        ...
        title_label = QLabel("Иконки и тайтлбар")
        title_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        title_label.setStyleSheet("background-color: transparent; font-size: 18px;")
        layout.addWidget(title_label)

        # Чекбокс для включения градиента
        checkbox = CustomToggle(f'Использовать градиент для иконок')
        checkbox.setStyleSheet("background-color: transparent")
        checkbox.stateChanged.connect(lambda state: self.parent.toggle_gradient("svg", state))
        ...
        solid_color_preview.mousePressEvent = lambda event: self.parent.choose_solid_color("svg")
        ...
        self.color1_preview.mousePressEvent = lambda event: self.parent.choose_gradient_color("svg", 1)
        ...
        self.color2_preview.mousePressEvent = lambda event: self.parent.choose_gradient_color("svg", 2)
        ...
        angle_slider.valueChanged.connect(lambda angle: self.parent.update_gradient_angle("svg", angle))
        gradient_layout.addWidget(angle_slider)
        angle_spin = QSpinBox()
        angle_spin.setStyleSheet("background: transparent")
        angle_spin.setRange(0, 360)
        angle_spin.setSuffix('°')
        angle_spin.valueChanged.connect(lambda angle: self.parent.update_gradient_angle("svg", angle))
        ...
        self.parent.gradient_settings["svg"]['widgets'] = {
            'checkbox': checkbox,
        ...
        self.parent.toggle_gradient("svg", checkbox.isChecked())