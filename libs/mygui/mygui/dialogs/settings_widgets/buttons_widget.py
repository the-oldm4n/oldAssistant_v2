...
class ButtonsWidget(QWidget):
    ...
    def init_ui(self):
        ...
        checkbox.stateChanged.connect(lambda state: self.parent.toggle_gradient("buttons", state))
        ...
        solid_color_preview.mousePressEvent = lambda event: self.parent.choose_solid_color("buttons")
        ...
        self.color1_preview.mousePressEvent = lambda event: self.parent.choose_gradient_color("buttons", 1)
        ...
        self.color2_preview.mousePressEvent = lambda event: self.parent.choose_gradient_color("buttons", 2)
        ...
        angle_slider.valueChanged.connect(lambda angle: self.parent.update_gradient_angle("buttons", angle))
        gradient_layout.addWidget(angle_slider)
        angle_spin = QSpinBox()
        angle_spin.setStyleSheet("background: transparent")
        angle_spin.setRange(0, 360)
        angle_spin.setSuffix('°')
        angle_spin.valueChanged.connect(lambda angle: self.parent.update_gradient_angle("buttons", angle))
        ...
        self.buttons_border_checkbox.setChecked(self.parent.border_in_buttons)
        ...
        self.parent.gradient_settings["buttons"]['widgets'] = {
            'checkbox': checkbox,
        ...
        self.parent.toggle_gradient("buttons", checkbox.isChecked())

    def on_border_btn_state_changed(self):
        """Обновляет внутренние переменные и применяет превью"""
        self.parent.border_in_buttons = self.buttons_border_checkbox.isChecked()
