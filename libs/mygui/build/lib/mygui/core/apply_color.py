"""Класс для управления цветами и стилями"""
import json
import re
...
class ApplyColor():
    ...
    def init(self, parent=None):
        """Явная инициализация после настройки конфига"""
        if self._initialized:
            return
            
        self.color_path = mygui_config.colors_path
        self.styles = self.load_styles()
        self.update_style_selector()
        self._initialized = True

    def get_stylesheet_by_key(self, key: str) -> str:
        """
        Возвращает CSS-строку для заданного ключа из self.styles.
        Пример: get_stylesheet_by_key("ItemCard") → "background-color: #0a1a12; border: 1px solid ..."
        """
        style_dict = self.styles.get(key, {})
        if not style_dict:
            return ""
        return "; ".join(f"{prop}: {value}" for prop, value in style_dict.items()) + ";"

    def apply_style_to_widget(self, widget: QWidget, key: str):
        """
        Применяет стиль из self.styles[key] напрямую к виджету через setStyleSheet.
        """
        css = self.get_stylesheet_by_key(key)
        ...
        try:
            ...
            if self.gradient_data == None:
                if "BasedColors" in self.styles and "svg" in self.styles["BasedColors"]:
                    ...
                    if hasattr(svg_widget, 'applyColorEffect'):
    ...
    def apply_modern_progress(self, key="svg", widget=None):
        try:
            color = self.get_color_property(_property=key)
            # widget.set_progress_color(color)

            if self.is_gradient(color):
                gradient = self.parse_gradient(color, widget.width(), widget.height())
                widget.set_progress_gradient(gradient)
            else:
                widget.set_progress_color(color)
    ...
    def get_color_property(self, _property="svg"):
        try:
            if _property == "svg":
                style = self.styles["BasedColors"]
                value = style.get("svg", "")
            elif _property == "border":
                style = self.styles["BasedColors"]
                value = style.get("border", "")
            else:
                raise ValueError(f"_property must be 'svg', 'border', not {_property}")
            ...
            return color_match.group(0) if color_match else "#B40000"
    ...
    def get_gradient_darker(self, key="svg"):
        color_or_gradient = self.get_color_property(key)
        return self.adjust_gradient_border(color_or_gradient, only_first_color=False)
    
    def apply_progressbar(self, key="svg", widget=None, style="solid"):
        """
    ...
    def get_raw_color(self):
        """Получает цвет стилей без обработки"""
        if "BasedColors" in self.styles and "svg" in self.styles["BasedColors"]:
